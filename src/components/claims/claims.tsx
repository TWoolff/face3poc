    ...
    if (query && filteredWords.length >= 1) {
        ...
        data.forEach((item) => {
            ...
            if (filteredSubcats?.length) {
                matchingSubcats.push(...filteredSubcats)
            }
        });
  ...
  return (
    <section className={filteredData.length === 1 ? `${css.claims} ${css.active}` : css.claims}>
      {dataToDisplay.map(({ id, title, description, cat, subcat, group }: IData) => (
        <Fragment key={id}>
          <Claim {...{ id, title, cat, subcat, query, keywords, description, group }} setQuery={setQuery} filteredWords={filteredWords} setFilteredWords={setFilteredWords} onClick={() => handleClaimClick(subcat)} className={filteredData.length === 1 ? `${css.claim} ${css.active}` : css.claim} />
        </Fragment>
      ))}
    </section>
  )
